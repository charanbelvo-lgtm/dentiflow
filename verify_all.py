...
def run_tests():
    with app.test_client() as client:
        ...
        for path in doctor_routes:
            resp = client.get(path, follow_redirects=True)
            assert resp.status_code == 200, f"Doctor route {path} failed with {resp.status_code}"
            print(f"  ✓ Doctor Route {path:<26} -> 200 OK")

        print("\n=== 2.1 TESTING AUTHENTICATED ADMIN ACCESS (ADMIN & FINANCIAL PAGES) ===")
        client.get('/demo-login/admin', follow_redirects=True)
        admin_routes = [
            '/billing',
            '/invoices/1/print',
            '/receipts/1/print',
        ...
        for path in admin_routes:
            resp = client.get(path, follow_redirects=True)
            assert resp.status_code == 200, f"Admin route {path} failed with {resp.status_code}"
            print(f"  ✓ Admin Route {path:<27} -> 200 OK")
        ...
        with app.app_context():
            pat = Patient.query.filter_by(email='budigeashwinigoud@gmail.com').first()
            pat_id = pat.id if pat else 1
            other_pat = Patient.query.filter(Patient.id != pat_id).first()
            other_id = other_pat.id if other_pat else 999

        resp = client.get(f'/patients/{pat_id}')
        assert resp.status_code == 200, "Patient accessing own profile failed"
        print(f"  ✓ Patient accessing own record /patients/{pat_id} -> 200 OK")

        # Patient attempting to view another patient's profile should return 403 Forbidden
        resp = client.get(f'/patients/{other_id}')
        assert resp.status_code == 403, f"Patient accessed other patient with code {resp.status_code}"
        print(f"  ✓ Patient accessing /patients/{other_id} -> 403 Forbidden (RBAC Protected!)")
